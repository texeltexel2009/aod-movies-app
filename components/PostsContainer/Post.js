    ...
    <article className="aspect-w-2 aspect-h-3 group" data-movie-id={id}>
      <Link href={`/${type}/${genSlug(title || name, id)}`}>
        <a className="flex w-full bg-white shadow-md rounded-lg overflow-hidden mx-auto cursor-pointer rounded-xl transform shadow-lg hover:shadow-2xl text-white">
          ...
          <div className="absolute inset-0 z-1 transition duration-300 ease-in-out bg-gradient-to-t from-[#000000cc] via-[#0000009e] to-transparent"></div>
          <div className="w-full absolute bottom-0 group z-1 p-6 md:p-10 space-y-3 md:space-y-6">
            <div className="align-self-end w-full">
              <div className=" space-y-3 md:space-y-6">
                <div className="flex flex-col space-y-2 inner">
                  <h1 className="text-lg md:text-2xl font-bold text-white" data-unsp-sanitized="clean">{title || name}</h1>
                </div>
                <div className="hidden md:flex flex-row justify-between text-xs md:text-sm text-xs md:text-sm">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="text-gray-400">Vote Average:</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <button type="button" className="absolute top-3 right-3 z-2 flex-shrink-0 rounded-full ml-3 p-1 border-2 border-white text-white hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-purple-500">
            <span className="sr-only">Add to favorites</span>
            <HeartIcon className="h-4 w-4 md:h-6 md:w-6" aria-hidden="true" />
          </button>
        </a>
      </Link>
    </article>
